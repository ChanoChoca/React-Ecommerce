                ...
                {cart?.map(({id, image, name, price, qty}, index) => (
                    <tr key={index}>
                        ...
                        <td className="d-grid justify-content-center gap-2">
                            <button className={styles.btn_pseudoclase} onClick={() => handleAddItem(id, price)}>Agregar
                                uno
                            </button>
                            <button className={styles.btn_pseudoclase}
                                    onClick={() => handleRemoveOneItem(id, price, qty)}>Remover uno
                            </button>
                            <button className={styles.btn_pseudoclase}
                                    onClick={() => handleRemoveItem(id, price, qty)}>Remover item
                            </button>
                        </td>
                    </tr>
                ))}
            ...
            <Form className={"py-5 " + styles.form} onSubmit={handleSaveCart}>
                <div className={"container text-center"}>
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control type="text" name="name" placeholder="Ingresar nombre..." value={formData.name}
                                      onChange={handleOnChange}/>
                    </Form.Group>
                    <Form.Group className="mb-3">
                        <Form.Label className={"lead"}>Teléfono</Form.Label>
                        <Form.Control type="tel" name="tel" placeholder="Ingresar número telefónico..."
                                      value={formData.tel} onChange={handleOnChange}/>
                    </Form.Group>
                    <Form.Group className="mb-3">
                        <Form.Label className={"lead"}>Email</Form.Label>
                        <Form.Control type="email" name="email" placeholder="Ingresar email..." value={formData.email}
                                      onChange={handleOnChange}/>
                    </Form.Group>
